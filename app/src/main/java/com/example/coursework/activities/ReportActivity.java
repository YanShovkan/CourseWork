...
public class ReportActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String text = "Отчет по количеству выписанных рецептов для пациентов в период с " + dateFrom.getTime().getDate() + " / " + dateFrom.getTime().getMonth() + " / " + (dateFrom.getTime().getYear()+ 1900) + " по " + dateTo.getTime().getDate() + " / " + dateTo.getTime().getMonth() + " / " + (dateTo.getTime().getYear()+ 1900);
        ...
        button_report.setOnClickListener(
                v -> {
                    ...
                    try {
                        report.generatePdf(receiptLogic.getFilteredList(dateFrom.getTime().getTime(), dateTo.getTime().getTime()), dateFrom.getTime(), dateTo.getTime());
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
