...
public class ReceiptsActivity extends AppCompatActivity {
    ...
    public void onResume() {
        super.onResume();
        logic.open();
        fillTable(Arrays.asList("Дата назначения", "Дата получения", "Имя получателя"), logic.getFullList());
        logic.close();
    }
    ...
    void fillTable(List<String> titles, List<ReceiptModel> receipts) {
        ...
        for (ReceiptModel receipt : receipts) {
            ...
            customerLogic.open();
            CustomerModel customer = customerLogic.getElement(receipt.getCustomerId());
            customerLogic.close();
            TextView textViewCustomer = new TextView(this);
            textViewCustomer.setHeight(100);
            textViewCustomer.setTextSize(16);
            textViewCustomer.setText(customer.getName());
            textViewCustomer.setTextColor(Color.WHITE);
            textViewCustomer.setGravity(Gravity.CENTER);
            ...
            tableRow.addView(textViewCustomer);
